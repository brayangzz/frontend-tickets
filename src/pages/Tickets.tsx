        ...
        <div className="flex gap-2 w-full lg:w-auto shrink-0 pr-1">
          ...
          <div className="relative" ref={statusRef}>
            <motion.button
              ...
              className={`flex items-center gap-3 h-12 px-6 rounded-full border bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all shadow-sm ${isStatusOpen ? "border-blue-500 ring-2 ring-blue-500/20" : "border-slate-200 dark:border-slate-700"}`}
            >
              <span className="font-bold text-sm text-slate-700 dark:text-slate-200">
                {filterStatus === "all" ? "Estatus" : statuses.find((s) => s.iIdStatus === Number(filterStatus))?.sStatus}
              </span>
              <span className="material-symbols-rounded text-slate-400" style={{ transform: isStatusOpen ? "rotate(180deg)" : "none" }}>expand_more</span>
            </motion.button>
            <AnimatePresence>
              {isStatusOpen && (
                <motion.div
                  initial={{ opacity: 0, y: -10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.95 }} transition={{ duration: 0.2 }}
                  className="absolute right-0 mt-2 w-56 bg-white dark:bg-[#1e293b] border border-slate-200 dark:border-slate-700 rounded-2xl shadow-xl overflow-hidden z-[100] p-2"
                >
          ...
          <div className="relative" ref={branchRef}>
            <motion.button
              ...
              className={`flex items-center gap-3 h-12 px-6 rounded-full border bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all shadow-sm ${isBranchOpen ? "border-blue-500 ring-2 ring-blue-500/20" : "border-slate-200 dark:border-slate-700"}`}
            >
              <span className="font-bold text-sm text-slate-700 dark:text-slate-200">
                {filterBranch === "all" ? "Sucursal" : branches.find((b

                  
                ) => b.iIdBranch === Number(filterBranch))?.sBranch}
              </span>
